...
class Generator(object):
    def __init__(self, num_vocabulary, batch_size, emb_dim, hidden_dim,
        ...
        # print(tf.shape(self.h0)) # prints Tensor("Shape:0", shape=(3,), dtype=int32)
        self.h0 = tf.stack([self.h_0, self.c_0])
        self.previous_sentence_state = tf.placeholder_with_default(input=self.h0, shape=[2, batch_size, emb_dim])
        ...
        _, _, self.context_last_state, self.g_predictions = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3: i < self.sequence_length,
            body=_pretrain_recurrence,
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(self.g_embeddings, self.start_token),
                       self.previous_sentence_state, g_predictions))
        ...
        def get_feature(input_x, name=''):
            # TODO: find out how much this representation relies on "being the same as in the discriminator"
            return self.discriminator.feature(input_x=input_x, name=name, custom_sequence_length=self.sequence_length)
        ...
        # TODO: do you only compare on the last sentence here? because in the Discriminator we compare all 5 sentences
    ...
    def generate(self, sess, batch):
        ...
        # initial state for sentence 0 of every story
        z_h0 = np.random.uniform(low=-.01, high=1, size=[self.batch_size, self.emb_dim])
        z_c0 = np.zeros(shape=[self.batch_size, self.emb_dim])
        prev_state = np.stack([z_h0, z_c0])

        # propagate state for the first 4 sentences
        for sentence_batch in batch[:4]:

            feed = {
                self.x: sentence_batch,
                self.previous_sentence_state: prev_state
            }

            # feed in context by using pre-train network, run on pretrain loop for the first 4 sentences
            # TODO: make sure this is run on pretrain! (is that declared by the parameters set in run?),
            #  also required to pass feed_dict feed or is this redundant?
            prev_state = sess.run(self.context_last_state, feed_dict=feed)

        # use state of first 4 sentences to generate last sentence
        h_prev = prev_state[0]
        c_prev = prev_state[1]
        feed = {
            self.h_0: h_prev,
        ...
        # generate last sentence with propagated last state.
        outputs, current_state = sess.run([self.gen_x, self.gen_last_state], feed_dict=feed)
        return outputs

    def pretrain_step(self, sess, x, hidden_state, cell_state):

        feed = {
            self.h_0: hidden_state,
            self.c_0: cell_state,
            self.x: x
        }

        # see format of outputs (is called to _, loss), should be a tuple
        _, loss, last_state = sess.run([self.pretrain_updates, self.pretrain_loss, self.context_last_state], feed_dict=feed)

        return loss, last_state

    def init_matrix(self, shape):
        return tf.random_normal(shape, stddev=5)  # try stddev 1.0 (as in orig impl.)?
