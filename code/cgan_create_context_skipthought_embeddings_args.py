...
# TO CREATE SKIPTHOUGHT EMBEDDINGS OF 4 CONTEXT SENTENCES OF GAN-GENERATED DATA
...
print("There are ", number_of_training_samples, " cgan training samples.")
...
cgan_context_sentences = list(map(lambda i: [" ".join(i)], cgan_context_sentences))
cgan_context_sentences = np.array(cgan_context_sentences)
print("shape of cgan data context: ", cgan_context_sentences.shape)
...
if LOAD_STORED_VAL_EMBEDDINGS:
    ...
    enc_cgan_ending_sentence2 = np.array(list(map(s.encoder.encode, cgan_ending_sentence2)))
    enc_cgan_context_sentences = np.array(list(map(s.encoder.encode, cgan_context_sentences)))
    print("Dimension of cgan context vector: ", enc_cgan_context_sentences.shape)
    enc_cgan_ending_sentence1 = np.array(list(map(s.encoder.encode, cgan_ending_sentence1)))

    # stack encoded cgan data to write to file
    stacked_endings = np.concatenate((enc_cgan_ending_sentence1, enc_cgan_ending_sentence2), axis=1)
    print("shape of enc_cgan_context_sentences: ", enc_cgan_context_sentences.shape)
    print("shape of stacked_endings: ", stacked_endings.shape)
    stacked_enc_cgan_data = np.concatenate((enc_cgan_context_sentences, stacked_endings), axis=1)

    # expected data shape: number_of_cgan_samples x 3 x 4800
    print("Dimension of stacked cgan data: ", stacked_enc_cgan_data.shape)
    # write to file
    flattened_cgan_data = np.ravel(stacked_enc_cgan_data)
    ...
    os.makedirs(os.path.dirname(enc_cgan_context_data_filename), exist_ok=True)
    np.savetxt(enc_cgan_context_data_filename, flattened_cgan_data, fmt='%1.10f')
